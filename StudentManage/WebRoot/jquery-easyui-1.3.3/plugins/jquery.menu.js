...
(function($) {
				...
				_a.children("div").each(function() {
					...
					if (_c.hasClass("menu-sep")) {
						...
						var _d = $.extend({}, $.parser.parseOptions(this, [
												"name", "iconCls", "href"]), {
									disabled : (_c.attr("disabled")
											? true
											: undefined)
								});
						_c.attr("name", _d.name || "").attr("href",
								_d.href || "");
	...
	function _11(_13, _14) {
		...
		_14._outerWidth(Math
				.max((_14[0].originalWidth || 0), _16, _15.minWidth));
		_14.css("display", d);
	};
	...
	function _24(_25, _26) {
		...
		if (_28.hasClass("menu-top")) {
			...
			if (_27 + _28.outerWidth() > $(window)._outerWidth()
					+ $(document)._scrollLeft()) {
				_27 = $(window)._outerWidth() + $(document).scrollLeft()
						- _28.outerWidth() - 5;
			}
			if (top + _28.outerHeight() > $(window)._outerHeight()
					+ $(document).scrollTop()) {
				top -= _28.outerHeight();
			}
		} else {
			...
			if (_27 + _28.outerWidth() + 5 > $(window)._outerWidth()
					+ $(document).scrollLeft()) {
		...
		_28.show(0, function() {
					if (!_28[0].shadow) {
						_28[0].shadow = $("<div class=\"menu-shadow\"></div>")
								.insertAfter(_28);
					}
					_28[0].shadow.css({
								display : "block",
	...
	function _37(_38, _39) {
		...
		if (_39.parent) {
			if (!_39.parent.submenu) {
				var _3b = $("<div class=\"menu\"><div class=\"menu-line\"></div></div>")
						.appendTo("body");
				_3b.hide();
		...
		if (_39.iconCls) {
			$("<div class=\"menu-icon\"></div>").addClass(_39.iconCls)
					.appendTo(_3c);
		}
		if (_39.id) {
			_3c.attr("id", _39.id);
					...
					if (_46) {
						...
						_46 = $.data(this, "menu", {
									options : $.extend({}, $.fn.menu.defaults,
											$.fn.menu.parseOptions(this), _44)
								});
						_1(this);
					}
					$(this).css({
								left : _46.options.left,
								top : _46.options.top
							});
				});
	};
	$.fn.menu.methods = {
		...
		setIcon : function(jq, _48) {
			return jq.each(function() {
						...
						if (_49.iconCls) {
							$(_49.target).children("div.menu-icon")
									.removeClass(_49.iconCls)
									.addClass(_48.iconCls);
						} else {
							$("<div class=\"menu-icon\"></div>")
									.addClass(_48.iconCls).appendTo(_48.target);
						}
					});
		},
