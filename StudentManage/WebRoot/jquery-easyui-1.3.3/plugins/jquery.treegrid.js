		...
		if (_a4) {
			if (_a4.children.length == 0) {
				...
				var _a5 = tr.children("td[field=" + _a3.treeField + "]")
						.children("div.datagrid-cell");
				_a5.find(".tree-icon").removeClass("tree-folder")
						.addClass("tree-file");
	...
	$.fn.treegrid.methods = {
		...
		reload : function(jq, id) {
			return jq.each(function() {
						if (id) {
							...
							var _af = $(this).datagrid("getPanel")
		...
		reloadFooter : function(jq, _b0) {
			return jq.each(function() {
						...
						if (_b1.showFooter) {
							_b1.view.renderFooter.call(_b1.view, this,
									dc.footer1, true);
							_b1.view.renderFooter.call(_b1.view, this,
									dc.footer2, false);
		...
		update : function(jq, _b6) {
			return jq.each(function() {
						...
						_b7.view.updateRow
								.call(_b7.view, this, _b6.id, _b6.row);
					});
		},
	...
	var _b9 = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_ba, _bb, _bc) {
			...
			function _c3(_c4, _c5, _c6) {
				...
				for (var i = 0; i < _c6.length; i++) {
					...
					var _ca = _bf + "-" + (_c4 ? 1 : 2) + "-"
							+ row[_bd.idField];
					_c7.push("<tr id=\"" + _ca + "\" node-id=\""
							+ row[_bd.idField] + "\" " + cls + " " + _c9 + ">");
					_c7 = _c7.concat(_c1.renderRow.call(_c1, _ba, _be, _c4,
							_c5, row));
					_c7.push("</tr>");
		...
		renderFooter : function(_cb, _cc, _cd) {
			...
			for (var i = 0; i < _cf.length; i++) {
				...
				_d1.push("<tr class=\"datagrid-row\" node-id="
						+ row[_ce.idField] + ">");
		...
		renderRow : function(_d2, _d3, _d4, _d5, row) {
			...
			for (var i = 0; i < _d3.length; i++) {
				...
				if (col) {
					var _d8 = col.styler
							? (col.styler(row[_d7], row) || "")
							: "";
					var _d9 = col.hidden
							? "style=\"display:none;" + _d8 + "\""
							: (_d8 ? "style=\"" + _d8 + "\"" : "");
					...
					if (col.checkbox) {
						...
						cc.push(" name=\"" + _d7 + "\" value=\""
								+ (row[_d7] != undefined ? row[_d7] : "")
								+ "\"/>");
					} else {
						var val = null;
						if (col.formatter) {
		...
		sort : function(_ee, _ef) {
			...
			function _f2(_f3) {
				_f3.sort(function(r1, r2) {
							return _f1(r1[_f0.sortName], r2[_f0.sortName])
									* (_f0.sortOrder == "asc" ? 1 : -1);
						});
		...
		finder : {
			getTr : function(_101, id, type, _102) {
				...
				if (_102 == 0) {
					...
					if (type == "body") {
						var tr = $("#" + $.data(_101, "datagrid").rowIdPrefix
								+ "-" + _102 + "-" + id);
						if (!tr.length) {
							tr = (_102 == 1 ? dc.body1 : dc.body2)
									.find("tr[node-id=" + id + "]");
						}
						return tr;
					} else {
						if (type == "footer") {
							return (_102 == 1 ? dc.footer1 : dc.footer2)
									.find("tr[node-id=" + id + "]");
						} else {
							if (type == "selected") {
								return (_102 == 1 ? dc.body1 : dc.body2)
