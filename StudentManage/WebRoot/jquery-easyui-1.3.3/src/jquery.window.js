...
(function($) {
	...
	function hcenter(target, tomove) {
		...
		if (opts.inline) {
			...
			opts.left = ($(window)._outerWidth() - width) / 2
					+ $(document).scrollLeft();
		}
		if (tomove) {
			moveWindow(target);
	...
	function vcenter(target, tomove) {
		...
		if (opts.inline) {
			...
			opts.top = ($(window)._outerHeight() - height) / 2
					+ $(document).scrollTop();
		}
		if (tomove) {
			moveWindow(target);
	...
	function create(target) {
		...
		if (state.options.modal == true) {
			...
			state.mask.css({
						width : (state.options.inline ? state.mask.parent()
								.width() : getPageArea().width),
						height : (state.options.inline ? state.mask.parent()
