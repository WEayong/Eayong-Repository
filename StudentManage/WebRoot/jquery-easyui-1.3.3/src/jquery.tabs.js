...
(function($) {
	...
	function createTab(container, pp, options) {
		...
		opts.tab.unbind('.tabs').bind('click.tabs', {
					...
					state.options.onContextMenu.call(container, e, $(this)
									.find('span.tabs-title').html(),
							getTabIndex(container, e.data.p));
				});
	...
	function updateTab(container, param) {
		...
		if (opts.closable) {
			...
			var a_close = $('<a href="javascript:void(0)" class="tabs-close"></a>')
					.appendTo(tab);
		...
		if (opts.tools) {
			var p_tool = $('<span class="tabs-p-tool"></span>').insertAfter(tab
					.find('a.tabs-inner'));
			if (typeof opts.tools == 'string') {
