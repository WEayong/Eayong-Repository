	...
	function buildGrid(target) {
		...
		$(target).datagrid($.extend({}, opts, {
					...
					onClickRow : function(index, row) {
						...
						if (opts.editIndex != index && row.editor) {
							...
							$(this).datagrid('getEditors', index)[0].target
									.focus();
					...
					onLoadSuccess : function(data) {
						...
						$(target).datagrid('getPanel')
								.find('div.datagrid-group').attr('style', '');
		...
		$(document).unbind('.propertygrid').bind('mousedown.propertygrid',
				function(e) {
					...
					// var p =
					// $(e.target).closest('div.propertygrid,div.combo-panel');
					if (p.length) {
						return;
					}
					stopEditing(currTarget);
		...
		render : function(target, container, frozen) {
			...
			for (var i = 0; i < groups.length; i++) {
				...
				if (!frozen) {
					...
					table.push(opts.groupFormatter.call(target, group.fvalue,
							group.rows));
					table.push('</span>');
				}
				table.push('</td>');
				...
				for (var j = 0; j < group.rows.length; j++) {
					...
					var cls = (index % 2 && opts.striped)
							? 'class="datagrid-row datagrid-row-alt"'
							: 'class="datagrid-row"';
					var styleValue = opts.rowStyler ? opts.rowStyler.call(
					...
					table.push('<tr id="' + rowId + '" datagrid-row-index="'
							+ index + '" ' + cls + ' ' + style + '>');
					table.push(this.renderRow.call(this, target, fields,
							frozen, index, group.rows[j]));
					table.push('</tr>');
					index++;
				}
				table.push('</tbody></table>');
			...
			view.find('div.datagrid-group').each(function() {
				...
				$(this).find('span.datagrid-row-expander').bind('click', {
							...
							if ($(this).hasClass('datagrid-row-collapse')) {
								$(target).datagrid('collapseGroup',
										e.data.groupIndex);
							} else {
								$(target).datagrid('expandGroup',
										e.data.groupIndex);
							}
						});
			});
